...
async function tryHaiku(systemPrompt, userPrompt, apiKey) {
  ...
  if (!response.ok) throw new Error(data.error?.message || 'Haiku error ' + response.status);
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (ANTHROPIC_KEY) {
      try {
        text = await tryHaiku(systemPrompt, userPrompt, ANTHROPIC_KEY);
        source = 'haiku';
        console.log('[REBT] Haiku OK');
      } catch (haikuErr) {
        console.log('[REBT] Haiku failed:', haikuErr.message, '-> falling back to Groq');
      }
    }
    ...
    if (!text && GROQ_KEY) {
      try {
        ...
        return {
          ...
          body: JSON.stringify({ error: 'Both Haiku and Groq failed' })
        };
      }
    }
